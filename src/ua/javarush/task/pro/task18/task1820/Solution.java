...
public class Solution {
    ...
    public static Optional<Car> getMostExpensiveCar(Stream<Car> cars) {
        ...
        Optional<Car> optional = cars.max(Comparator.comparingInt(Car::getPrice));
        return optional;
    }

    public static Optional<Car> getMoreExpensiveCar(Stream<Car> cars, Car mostExpensiveCar) {
        //напишіть тут ваш код
        Stream<Car> stream = cars.filter(car -> car.getPrice() > mostExpensiveCar.getPrice());
        Optional<Car> optional = stream.findFirst();
        return optional;
    }
}
